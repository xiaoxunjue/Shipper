 ...
 */
public class EditPingTaiFragment extends BaseFragment {
...
//        });
    ...
    public static EditPingTaiFragment newInstance(Bundle bundle) {
        editgoodsinfo = (GoodsInfoEditBean) bundle.getParcelable("editgoodsinfo");
        EditPingTaiFragment fragment = new EditPingTaiFragment();
...
//    }
    ...
    private void geteditgoods() {


        GoodsInfoEditBean bean = editgoodsinfo;
        if (Integer.valueOf(bean.getCode()) == 0) {
            if (bean.getData().getInfo().getIsPlatform() == 1) {
                goodsid = String.valueOf(bean.getData().getInfo().getId());
